 ...
 */
function getPath(curFileName, desFileName, defaultSettings) {
    ...
    if (filename.startsWith('./') || filename.startsWith('../')) {
        ...
        // 绝对路径设置文件前缀
        filename = setPathPrefix(desFileName, defaultSettings, curFileName);
    }
    return filename;
 ...
 */
function setPathPrefix(desFileName, defaultSettings) {
    const aliasMappings = defaultSettings.aliasMappings;
...
// 补全文件后缀并且查看文件是否存在
function addSuffix(desFileName, defaultSettings) {
    const extName = path.extname(desFileName);
    const filesExt = defaultSettings.matchFileExt;
...
// 设置绝对路径
function setFullNames(sourceArr, defaultSettings) {
    let targetArr = [];
    sourceArr.forEach(name => {
        if (path.isAbsolute(name)) {
            targetArr.push(addSuffix(name, defaultSettings));
        } else {
            targetArr.push(name);
        }
    })
