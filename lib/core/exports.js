...
module.exports = function(rootContainer) {
    ...
    totalFiles =  [...new Set(setFullNames(setRelativeNames(totalFiles)))];
    // importFiles = [...new Set(setFullNames(setRelativeNames(importFiles), defaultSettings))];
    importFiles = [...new Set(setFullNames(setRelativeNames(importFiles)))];
    // exportsFiles 理论和实际上 不需要做处理，需要去重
    exportsFiles = [...new Set(setFullNames(setRelativeNames(exportsFiles)))];
