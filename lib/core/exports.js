 ...
 */
module.exports = function(rootContainer, defaultSettings) {
    // 导入文件名称
    let importFiles = [];
    // 导出文件名称
    let exportsFiles = [];
    ...
    const ignorePaths = defaultSettings.ignorePaths;
    ...
    totalFiles =  [...new Set(setFullNames(setRelativeNames(totalFiles), defaultSettings))];
    // importFiles = [...new Set(setFullNames(setRelativeNames(importFiles), defaultSettings))];
    importFiles = [...new Set(setFullNames(setRelativeNames(importFiles), defaultSettings))];
    // exportsFiles 理论和实际上 不需要做处理，需要去重
    exportsFiles = [...new Set(setFullNames(setRelativeNames(exportsFiles), defaultSettings))];
